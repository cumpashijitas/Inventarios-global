...
import {
  AccionesDocumento, DocCotizacion, ReciboVenta, textoWhatsappCotizacion, textoWhatsappVenta,
...
export default function NuevaVentaPage() {
  ...
  const reciboVentaRef = useRef<HTMLDivElement>(null);
  const reciboCotRef   = useRef<HTMLDivElement>(null);
        ...
        <DialogContent className="max-w-sm overflow-y-auto max-h-[90vh]">
          <DialogHeader>
            <div className="flex items-center justify-between gap-2">
              ...
              {reciboVenta && (
                <AccionesDocumento
                  docRef={reciboVentaRef}
                  nombreArchivo={`venta_${reciboVenta.numero}`}
                  textoWhatsapp={textoWhatsappVenta(reciboVenta, empresa)}
                />
              )}
            </div>
          </DialogHeader>
          {reciboVenta && <ReciboVenta ref={reciboVentaRef} venta={reciboVenta} empresa={empresa} />}
        </DialogContent>
      </Dialog>
          ...
          <DialogHeader>
            <div className="flex items-center justify-between gap-2">
              ...
              {reciboCot && (
                <AccionesDocumento
                  docRef={reciboCotRef}
                  nombreArchivo={`cotizacion_${reciboCot.numero}`}
                  textoWhatsapp={textoWhatsappCotizacion(reciboCot, empresa)}
                />
              )}
            </div>
          </DialogHeader>
          {reciboCot && <DocCotizacion ref={reciboCotRef} cot={reciboCot} empresa={empresa} />}
        </DialogContent>
      </Dialog>
    </div>
  );
